...
const Transactions = () => {
  ...
  const [txns, setTxns] = useState([]); // Start empty, fill from DB
    ...
    const fetchTransactions = async () => {
      try {
        ...
        const formattedData = res.data.map((t) => ({
          id: t._id,
          name: t.description, // DB says 'description', UI says 'name'
  ...
  const handleExport = () => {
    ...
    const rows = txns.map(
      (t) => `${t.name},${t.date},${t.category},${t.amount},${t.status}`
    );
    const csvContent =
  ...
  // 2. ADD LOGIC (UPDATED TO SAVE TO DB)
  const handleAdd = async (e) => {
    ...
    try {
      ...
      // Prepare payload
      const body = {
        description: newTxn.name, // UI calls it 'name', DB expects 'description'
      ...
      // Create UI object from Server Response
      const savedTxn = {
        id: res.data._id,
      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-txt-secondary">
            Detailed history of your financial movements.
          </p>
        </div>
        <div className="flex gap-2">
          <button
            onClick={handleExport}
            className="flex items-center gap-2 px-4 py-2 bg-bg-card border border-border rounded-lg text-txt-secondary hover:text-txt-primary transition-all"
          >
            <Download size={18} /> Export
          </button>
          <button
            onClick={() => setIsModalOpen(true)}
            className="flex items-center gap-2 px-4 py-2 bg-brand text-white rounded-lg hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-500/30"
          >
      ...
      <div className="card-base p-4 mb-6">
        <div className="flex gap-4 items-center">
          <div className="relative flex-1">
            <Search
              className="absolute left-3 top-3 text-txt-secondary"
              size={20}
            />
            <input
              type="text"
          ...
          <button className="p-2.5 rounded-lg border border-border text-txt-secondary hover:bg-bg-secondary">
            <Filter size={20} />
          </button>
        </div>
      </div>
                ...
                >
                  ...
                  <td
                    className={`p-4 font-bold text-right ${
                      t.amount > 0 ? "text-green-500" : "text-txt-primary"
                    }`}
                  >
                    {t.amount > 0 ? "+" : ""}
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
            ...
            >
              ...
              <form onSubmit={handleAdd} className="p-6 space-y-4">
                <div>
                  <label className="text-xs font-bold text-txt-secondary uppercase">
                    Description
                  </label>
                  <input
                    required
                    className="w-full bg-bg-secondary border border-border rounded p-2 text-txt-primary focus:border-brand outline-none"
                    placeholder="e.g. Grocery"
                    value={newTxn.name}
                    onChange={(e) =>
                      setNewTxn({ ...newTxn, name: e.target.value })
                    }
                  />
                </div>
                ...
                <button className="w-full py-3 bg-brand text-white font-bold rounded-lg hover:shadow-lg transition-all mt-4">
                  Save Transaction
                </button>
              </form>
            </motion.div>
